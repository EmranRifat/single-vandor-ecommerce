...
export const BarcodeScanner: React.FC<Props> = ({ onScan, oneShot }) => {
  ...
  const [vibrationPermissionAsked, setVibrationPermissionAsked] =
    useState(false);
  ...
  const hasMediaDevices =
    isClient && typeof navigator !== "undefined" && !!navigator.mediaDevices;
  ...
  const hasVibration =
    isClient && typeof navigator !== "undefined" && !!navigator.vibrate;
  ...
  const handleScan = useCallback(
    (result: any) => {
      ...
      if (timeSinceLastScan < 300) {
        // 300ms debounce for faster scanning
        console.log(
          `📝 Debounced duplicate scan: ${barcode} (${timeSinceLastScan}ms ago)`,
        );
        ...
        if (isClient && vibrationEnabled && hasVibration) {
          try {
            ...
            if (!vibrationResult && (navigator as any).webkitVibrate) {
              vibrationResult = (navigator as any).webkitVibrate([
                200, 100, 200,
              ]);
  ...
  const processManualScan = useCallback(
    (result: any) => {
      ...
      if (timeSinceLastScan < 300) {
        console.log(
          `📝 Manual scan debounced: ${barcode} (${timeSinceLastScan}ms ago)`,
        );
        return;
      }
  ...
  const handleManualScan = useCallback(async () => {
    if (!videoRef.current || !isActive) {
      console.log(
        "📷 Manual scan failed: video not ready or scanner not active",
      );
      return;
    }
  ...
  return (
    ...
    >
      ...
      {(!hasMediaDevices || !hasGetUserMedia || !isSecureContext) && (
        ...
        >
          <div>
            <strong>⚠️ Browser Compatibility Issues:</strong>
          </div>
          <ul
            style={{ margin: "8px 0", paddingLeft: "20px", fontSize: "13px" }}
          >
      ...
      <audio ref={audioRef} preload="auto">
        ...
        <source
          src="data:audio/wav;base64,UklGRnoGAABXQVZFZm10IBAAAAABAAEAQB8AAEAfAAABAAgAZGF0YQoGAACBhYqFbF1fdJivrJBhNjVgodDbq2EcBj+a2/LDciUFLIHO8tiJNwgZaLvt559NEAxQp+PwtmMcBjiR1/LMeSwFJHfH8N2QQAoUXrTp66hVFApGn+DyvmocATV+zPD3dzkJCVy07aJYFglCoN3wqA=="
          type="audio/wav"
        />
        <source
          src="data:audio/ogg;base64,T2dnUwACAAAAAAAAAADqnjMlAAAAAOyyzPIBHgF2b3JiaXMAAAAAAUAfAABAHwAAQB8AAEAfAACZAU9nZ1MAAAAAAAAAAAAAOp4zJQEAAAAGlwAADnZvcmJpc0QAAABYaXBoLk9yZwqBCV9wb3J0aW9uAQAAAQAAAAEAAQAAAQAAAAEAAQABAAAAAA=="
          type="audio/ogg"
        />
      </audio>
      ...
      {scanHistory.length > 0 ? (
            ...
            {scanHistory.map((scan, index) => (
              <div
                ...
                style={{
                  ...
                  borderRadius: "6px",
                  borderLeft:
                    index === 0 ? "4px solid #28a745" : "4px solid #6c757d",
                }}
              >
                <div
                  style={{
        ...
        >
          ...
          <small>
            Start scanning to see unique barcodes here. Duplicates will be
            ignored.
          </small>
        </div>
      )}
      ...
      {error && (
        ...
        >
          <div>
            ❌ <strong>Camera token Error:</strong>
          </div>
            ...
            (error.includes("getUserMedia") && (
              ...
              >
                ...
                <ul style={{ margin: "8px 0", paddingLeft: "20px" }}>
                  ...
                  <li>token the site via HTTPS (required for camera token)</li>
                  <li>
                    Check if camera permissions are blocked in browser settings
                  </li>
          ...
          {error.includes("HTTPS") && (
            ...
            >
              <strong>HTTPS Required:</strong> Camera token requires a secure
              connection. Please token this site via HTTPS or use localhost for
              development.
            </div>
          )}
        </div>
