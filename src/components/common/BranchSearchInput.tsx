...
export default function BranchSearchInput({
  ...
  const [searchBranchQueryParams, setSearchBranchQueryParams] =
    useState<SearchBranchQuery>({
      branch_query: "",
      page: 1,
      per_page: 100,
  ...
  const handleBranchSelectionChange = (selectedKey: React.Key | null) => {
    ...
    if (data && data.data && data.data.length !== 0 && selectedKey) {
      // console.log("selectedKey", selectedKey);
      const selectedBranch: Branch | undefined = data?.data.find(
        (branch: Branch) => branch.branch_code === selectedKey,
      );
      if (selectedBranch) {
      ...
      {(item: Branch) => (
        <AutocompleteItem key={item.branch_code} textValue={item.name}>
          <p className="text-postDark dark:text-postLight">
            ...
            {item.upzilla.length && item.upzilla != "0"
              ? `, ${item.upzilla}`
              : ""}{" "}
            {item.district.length && item.district != "0"
              ? `, ${item.district}`
              : ""}
          </p>
        </AutocompleteItem>
